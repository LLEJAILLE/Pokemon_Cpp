...
namespace rtype::ECS::Ecs3D {
    ...
    void drawTextureAndRecLoginScene(std::shared_ptr<IEntity> backgroundMenu, std::shared_ptr<IEntity> logo, std::shared_ptr<IEntity> close, std::shared_ptr<IEntity> usernameTextArea, std::shared_ptr<IEntity> passwordTextArea, std::shared_ptr<IEntity> login, std::shared_ptr<IEntity> registerBtn)
    {
        ...
        Rectangle rec = {registerBtn->getComponent<rtype::ECS::Ecs3D::ButtonMenu>()->button.x, registerBtn->getComponent<rtype::ECS::Ecs3D::ButtonMenu>()->button.y, registerBtn->getComponent<rtype::ECS::Ecs3D::ButtonMenu>()->button.width, registerBtn->getComponent<rtype::ECS::Ecs3D::ButtonMenu>()->button.height};

        DrawRectangleRounded(rec, 1, 0, BLACK);
        DrawText("Register", rec.x + 10, rec.y + 25, 15, WHITE);
    }
}